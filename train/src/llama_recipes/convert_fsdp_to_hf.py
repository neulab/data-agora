...
def main(
    ...
    api = HfApi()
    create_repo(consolidated_model_path, private=True, token=token)

    # Push the model and tokenizer to the Hub
    model.push_to_hub(consolidated_model_path, use_auth_token=token)
    tokenizer.push_to_hub(consolidated_model_path, use_auth_token=token)
